...
import { clear } from "../../../node_modules/@testing-library/user-event/dist/clear.js";
              ...
              <Dropdown.Select placeholder="Select the category">
                ...
                <Dropdown.List>
                  {categories.map((option) => (
                    <Dropdown.Option key={option.id} onClick={(option)=>handleSelectOption(option)}>
                      {option.name}
                    </Dropdown.Option>
                  ))}
                </Dropdown.List>
                {selectCategory && <span className="inline-block p-3 text-sm font-medium bg-green-200 rounded-lg color-green">{selectCategory.name}</span>}
            </Dropdown>
          </Field>
        </div>
